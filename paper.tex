...
The following tables \ref{test-1} \ref{test-2} \ref{test-3} describe a summary of the experiment results that have been performed with three different alpha values. The result is on all test cases, IS2 cases to IS4 cases were successfully detected by regression and critical path analysis is appropriate. It's just that for the SI1 case, only the alpha value is 0.1
...
very strong evidence to reject h0 but it means that h0 cannot be rejected by too sure. Conversely, if the smaller the significant value used, the smaller the value will be
the stronger the evidence needed to reject h0 which means that h0 can be rejected.
