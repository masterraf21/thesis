...
\begin{table}[]
	\begin{tabular}{llll}
		...
		\textbf{Service/Operation} & \textbf{Baseline} & \textbf{Regression} & \textbf{Difference} \\
		inventory/write-cache & 63.1 ms & 368 ms  & +305 ms  \\
		inventory-db/update   & 1.75 ms & 2.26 ms & +0.51 ms \\
		memcached/set         & 4.94 ms & 4.71 ms & -0.23 ms \\
		inventory/update      & 15.2 ms & 14.8 ms & -0.47 ms \\
		inventory/db-update   & 32 ms   & 30.6 ms & -1.4 ms 
	\end{tabular}
\end{table}
