...
The first case will have an ID with an SI prefix signifying the \textit{Internal-Scenario}.
The second type are cases that simulate regression by increasing the number of users and load on the Load Generator. The second case will have an ID with the SE prefix denotes an \textit{External-Scenario}. Table IV.5 will explain the cases
system testing.
