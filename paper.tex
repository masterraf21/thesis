...
If a regression is detected, the next step is to perform a critical path analysis to be done by calculating the contribution of the critical path data trace indicated that the regression occurred and will be compared with the critical path data derived from the baseline calculation. From this comparison, it will be sorted and the result of which operation has the largest latency change and becomes
