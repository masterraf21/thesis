...
There are two types of test cases to be tested on the system. first type
are cases that simulate regression by making changes that
occurs at the application level by making modifications intentionally at the code level.
The first case will have an ID with an SI prefix signifying the internal-scenario.
The second type are cases that simulate regression by increasing
IV-11
number of users and load on Load Generator. The second case will have an ID with
the SE prefix denotes an external-scenario. Table IV.5 will explain the cases
system testing.
