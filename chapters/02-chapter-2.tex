...
Gambar \ref{ch2-cp-3} menunjukkan contoh \textit{trace} kedua. Pada contoh ini, dua \textit{span} merepresentasikan pekerjaan yang sama namun dilihat berbeda oleh \textit{client} (\textit{client} A) dan \textit{server} (\textit{server} A).
\begin{figure}[htb]
	\centering
