...
Gambar \ref{ch2-cp-2} menunjukkan contoh \textit{trace} kedua. Pada contoh ini, dua \textit{span} merepresentasikan pekerjaan yang sama namun dilihat berbeda oleh \textit{client} (\textit{client} A) dan \textit{server} (\textit{server} A)
\begin{figure}[htb]
	\centering
	\includegraphics[width=1\textwidth]{resources/ch2/cp-2.png}
	\caption{\textit{Trace} dengan \textit{span} milik \textit{client} pada \textit{critical path}} \citep{parker2020distributed}}
	\label{ch2-cp-2}
\end{figure}
