...
\begin{small}
	\begin{longtable}{ | p{10cm} | p{2cm} | p{2cm} | p{2cm} | }
		\caption{Kontribusi \textit{critical path} dalam \textit{trace} \textit{baseline} dan regresi}
		\label{corr-tab-2}                                                           
		\\ \hline
		\centering\bfseries{\textit{Service} / Operasi} & \centering\bfseries{\textit{Baseline}} & \centering\bfseries{\textit{Regresi}} & \centering\bfseries{\textit{Perubahan}} \tabularnewline \hline
		\endfirsthead
		%			\hline
		inventory/write-cache & 63.1 ms & 368 ms & $\num{+305}$ ms \\ \hline
%		\textbf{runinfo.host}: vm73 & 0.41 \\ \hline
%		\textbf{service}: inventory, \textbf{service.version}: 1.14.1 &  $\num{-0.65}$ \\ \hline
	\end{longtable}
\end{small}
