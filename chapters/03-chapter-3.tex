...
Dari pemaparan alur sistem \textit{Performance Regression Analysis} pada subbab \ref{analisis-masalah}, dapat dirumuskan beberapa kriteria sistem \textit{distributed tracing}. Daftar kriteria dapat dilihat pada Tabel \ref{ch3-trace-crit}.
\begin{small}
	\begin{longtable}{ | p{3cm} | p{10cm} |}
		...
		\endfirsthead
		RQ-1 & Komponen instrumentasi dapat menyediakan informasi induk \textit{span} dan anak \textit{span} antar \textit{service} sehingga dapat terlihat konteks \textit{request} dalam \textit{span} secara menyeluruh  \\ \hline
		RQ-2 & Komponen \textit{deployment} menyediakan dukungan untuk Kubernetes \\ \hline
		RQ-3 & Komponen \textit{deplyoment} menyediakan dukungan asli untuk \textit{storage} eksternal \\ \hline
	\end{longtable}
\end{small}


Dari berbagai jenis solusi \textit{distributed tracing} yang sudah ada seperti pada subbab \ref{terkait}, penulis menilai Zipkin merupakan solusi yang tepat untuk digunakan sebab memenuhi kriteria pada tabel di atas.
