...
\begin{small}
	\begin{longtable}{ | p{1cm} | p{2cm} | p{3cm} | p{7.5cm} | }
		...
		\endfirsthead
		EP-1 & \centering{GET} & \texttt{/state} & Data \textit{state} dari \textit{engine} yang akan berisikan informasi mengenai status pendeteksian regresi, pengecekan terakhir regresi, dan hasil analysis \textit{critical path} yang diduga menjadi penyebab regresi \\ \hline
	\end{longtable}
\end{small}
