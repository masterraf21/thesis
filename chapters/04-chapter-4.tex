...
Gambar \ref{result_log_1} menunjukkan \textit{log} yang didapatkan dari kasus SI1 dengan nilai signifikan 0,05. Terlihat \texttt{pvalue} yang dihasilkan dari tes bernilai 0,067. Pada tes tersebut, nilai signifikan yang digunakan adalah 0,05 sehingga berdasarkan algoritma, pada kasus tersebut tidak bisa langsung menolak $h_{0}$ dan harus dilanjutkan membandingkan nilai \texttt{statistic} dengan nilai \texttt{critical value}, dan hasilnya menyatakan bahwa CDF \textit{latency} pada kasus tersebut memiliki distribusi yang sama dengan \textit{baseline}, sehingga regresi tidak terdeteksi. Hal yang sama akan berlaku untuk seluruh nilai signifikan $<0,05$, namun pada pengujian dengan nilai signifikan 0,1, kasus SI1 akan terdeteksi sebagai regresi.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=1\textwidth]{resources/ch4/log/1-log-new.png}
	\caption{\textit{Log} hasil pengujian kasus SI1 dengan alpha 0,05}
	\label{result_log_1}
\end{figure}

Dapat diartikan dari analsiis di atas, penggunaan nilai signifikan pada PRA Engine akan berpengaruh pada hasil terutama jika kasus regresi yang ingin dideteksi tidak memiliki perubahan yang sangat berarti pada nilai \textit{latency}-nya seperti yang terdapat pada kasus SI1. Dapat disimpulkan bahwa semakin besar nilai signifikan, akan semakin sensitif juga algoritma dalam mendeteksi regresi yang terjadi sebab tidak diperlukan bukti yang sangat kuat untuk menolak $h_{0}$ namun artinya $h_{0}$ tidak dapat ditolak dengan terlalu yakin. Sebalknya jika semakin kecil nilai signifikan yang digunakan akan semakin kuat juga bukti yang diperlukan untuk menolak $h_{0}$ dengan arti $h_{0}$ dapat ditolak dengan semakin yakin.

Pada kasus faktor perubahan internal lainnya selain SI1, semua hasil pengujian dengan ketiga nilai signifikan dapat berbasil dideteksi dan ditentukan sumber penyebabnya sebab algoritma dapat menentukan bahwa CDF data \textit{latency} kasus regresi memiliki distribusi yang berbeda dengan CDF data \textit{latency} \textit{baseline}. Sementara pada kasus-kasus yang menyimulasikan perubahan dari eksternal, yaitu kasus SE1 sampai SE4, PRA Engine juga dapat mendeteksi terjadinya regresi namun hasil analisis \textit{critical path} tidak dapat menentukan sumber terjadinya regresi karena memang regresi tidak diakibatkan oleh perubahan pada level \textit{service}.

Dapat disimpulkan bahwa algoritma belum dapat menentukan apakah regresi yang terjadi diakibatkan oleh perubahan yang terjadi pada level aplikasi ataupun perubahan yang terjadi di luar aplikasi seperti peningkatan jumlah pengguna ataupun peningkatan \textit{load} pada sebagian operasi. Namun jika dari analisis \textit{critical path} dapat terlihat selisih \textit{latency} dari operasi yang melebihi \textit{threshold} tertentu dapat menjadi indikasi bahwa regresi yang terjadi diakibatkan oleh perubahan di level aplikasi. Sebaliknya, jika analisis \textit{critical path} tidak dapat menentukan operasi yang menjadi kandidat penyebab regresi karena selisihnya tidak lebih besar daripada \textit{threshold}, maka dapat menjadi indikasi bahwa regresi yang terdeteksi diakibatkan oleh perubahan yang disebabkan oleh faktor eksternal.
