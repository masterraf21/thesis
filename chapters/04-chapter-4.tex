...
Pengujian pada tabel \ref{testcase} seluruhnya dilakukan dengan cara terlebih dahulu melakukan \textit{deployment} masing-masing kasus uji di lingkungan Kubernetes kemudian menjalankan aplikasi Load Generator sesuai dengan spesifikasi masing-masing kasus uji selama $\pm 5$ menit. Pengujian akan dilakukan dengan memanggil \textit{endpoint} \texttt{/analysis/range} dengan parameter StartTime dan EndTime yang sesuai dengan kasus uji. Untuk keperluan pengujian akan digunakan 3 level signifikan atau alpha yang berbeda yaitu 0.1, 0.05, dan 0,001. Ketiga level signifikan ini digunakan untuk mengetahui sensitivitas hasil pendeteksian regresi dengan arti semakin kecil nilai signifikan, maka akan semakin kuat bukti untuk menolak $h_{0}$. Hasil akhir dari setiap kasus uji akan berupa \textit{response} dalam bentuk JSON dari API dan juga \textit{log} dari aplikasi untuk mengetahui hasil tes  statistik K-S yang akan dipergunakan untuk keperluan analisis selanjutnya.
\begin{table}[htb]
...
Pada kasus ini, regresi terdeteksi pada nilai alpha 0.1 namun tidak terdeteksi pada nilai alpha 0.05 dan 0.001. Analisis \textit{critical path} dapat menentukan sumber regresi pada nilai alpha 0.1 yaitu operasi \texttt{placeorder} dari \textit{service} \texttt{CheckoutService} dengan selisih perbedaan \textit{latency} sebesar 127ms.
