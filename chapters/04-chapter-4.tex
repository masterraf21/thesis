...
\begin{algorithm}[hbt!]
	...
	\caption{Algoritme transformasi data \textit{trace} menjadi fungsi distribusi kumulatif}\label{alg:crit-analysis}
\end{algorithm}
...
\begin{algorithm}[hbt!]
	...
	\caption{Algoritme transformasi data \textit{trace} menjadi \textit{critical path}}\label{alg:crit-analysis}
\end{algorithm}
...
\begin{small}
	\begin{longtable}{ | p{1cm} | p{3cm} | p{10cm} | }
		...
		\endfirsthead
		UI-1 & zipkin (Pengambilan Data) & Modul ini bertanggung jawab untuk mengambil data dari API Zipkin yang memiliki data hasil \textit{trace} dari aplikasi. Selain mengambil data \textit{trace} dari Zipkin, modul ini juga bertanggung jawab untuk mengambil model-model \textit{baseline} yang terdapat pada \textit{storage}. \\ \hline
