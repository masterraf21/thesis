...
%tren grpc sendiri 
Di sisi lain, terdapat peningkatan tren penggunaan gRPC \citep{grpc} sebagai cara berkomunikasi antar \textit{service} pada aplikasi berbasis Microservice.  gRPC sendiri merupakan salah satu metode implementasi \textit{remote procedure call} (RPC) yang merupakan hasil inkubasi dari Cloud Native Computing Foundation (CNCF) yang menawarkan keunggulan kinerja dibandingkan cara berkomunikasi dengan gaya REST. Mengingat gRPC merupakan suatu protokol yang baru dan adopsi dalam lingkungan industri maupun akademisi belum seluas penggunaan protokol seperti REST, sehingga belum banyak eksplorasi mengenai \textit{debugging} pada protokol ini.
